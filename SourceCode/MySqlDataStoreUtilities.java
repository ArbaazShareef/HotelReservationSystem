...
try
	...
	//======================================
	public static HashMap<String,Products> getProductsforsales()
	{
		...
		HashMap<String,Products> finalp;	
		...
		finalp = new HashMap<String,Products>();
	...
	public static HashMap<Date, Integer> getDailyTransactionsFromMySQL()
	{
		HashMap<Date, Integer> dailyTransactions = new HashMap<Date, Integer>();
		try
		{
			...
			Statement s = conn.createStatement ();
			s.executeQuery ("SELECT * FROM customerorders ORDER BY CheckInDate ASC");
			ResultSet rs = s.getResultSet();
			Date  d = sdf.parse("10/15/17");
			int t = 100;
			dailyTransactions.put(d,t);
			while (rs.next ())
			{
				int price = rs.getInt("Price");
				Date orderDate = sdf.parse(rs.getString("CheckInDate"));
				int Total = price;
				//System.out.println("orderDate:"+orderDate);
				//System.out.println("orderTotal:"+price*itemQty);
				try{
							for(Map.Entry<Date,Integer> m :dailyTransactions.entrySet()){
								Date date =m.getKey();
						if(date.compareTo(orderDate) == 0){
							int total = m.getValue()+(price);
							dailyTransactions.put(m.getKey(),total);
						}else{	
								System.out.println("Inside Else : ");
							int total = price;
							dailyTransactions.put(orderDate,total);

						}
						Date Key = m.getKey();
						 Total = m.getValue();
					System.out.println("Daily Transactions :"+Key+Total);
				}
...

	{
		try
		{
			Connection conn = null;
			Class.forName("com.mysql.jdbc.Driver").newInstance();
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/smartportables?autoReconnect=true&useSSL=false", "root", "tanmaya");
...
//ID,Category,Retailer,Image,Name, Company, Conditions, Color,Description,Price,Accessories
	...
	public static void main(String args[]){
		HashMap<String,String>images = MySqlDataStoreUtilities.getImages("Trump Hotel");
